...
// POST /api/sync/products — Mahsulotni saqlash, rasm yangilash yoki yangi mahsulot qo'shish
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "sync_all" && Array.isArray(productsList)) {
      globalThis.__HOLVA_SERVER_PRODUCTS = productsList;
    ...
    if (action === "create" && product) {
      ...
      globalThis.__HOLVA_SERVER_PRODUCTS = [newProd, ...globalThis.__HOLVA_SERVER_PRODUCTS];
      return NextResponse.json({ success: true, products: globalThis.__HOLVA_SERVER_PRODUCTS });
    }
